...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-literature runtests.py tests")
...
def release(c, bumpsize=''):
    ...
    import django_literature
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_literature.__version__))
    c.run("git push --tags")
