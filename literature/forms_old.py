...
class CitationJSFormMixin(forms.Form):
    ...
    preview = forms.CharField(label=_("Preview"), required=False)
...
class DateVariableForm(forms.Form):
    ...
    date_parts = forms.RegexField(
        label=_("date"),
        regex=r"^\d{4}(-\d{2})?(-\d{2})?(\/(\d{4}(-\d{2})?(-\d{2})?)?)?$",
        help_text=_(
            'Date (e.g. "2019", "2019-06", "2019-06-15", "2019/" for an open ended range, "2019-06/2020-07" for a range)'
        ),
        required=False,
    )

    season = forms.CharField(
        label=_("value"), help_text=_("Season associated with the item (e.g. “Spring”)"), required=False
    )
    circa = forms.CharField(label=_("circa"), required=False)
